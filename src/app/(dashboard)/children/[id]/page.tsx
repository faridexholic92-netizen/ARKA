...
import { ArrowLeft, Edit2, TrendingUp, CalendarCheck, Trophy, Heart, User, FileText } from "lucide-react";
...
export default function ChildDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getGrowthRecords(child.id), getAttendance(child.id), getAchievements(child.id), getHealthRecords(child.id)])
      .then(([g, a, ach, h]) => { setGrowth(g); setAttendance(a); setAchievements(ach); setHealthRecords(h); setDataLoaded(true); });
  }, [child, dataLoaded]);
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <Link href={`/children/${id}/report`} className="flex items-center gap-1.5 bg-white border border-gray-200 text-gray-600 px-3 py-2 rounded-xl text-sm font-medium hover:bg-gray-50 transition">
      ...
      <div className="bg-white rounded-2xl border overflow-hidden mb-4">
        ...
        <div className="px-5 pb-5">
          ...
          <div className="mb-4">
            ...
            {child.icNumber && <p className="text-gray-500 text-sm mt-1 font-mono">🪪 {child.icNumber}</p>}
          </div>
        ...
        <div className="space-y-4">
          ...
          {(child.schoolName || child.schoolYear || child.schoolClass) && (
            <Section title="🏫 Sekolah / Tadika">
              <div className="col-span-2"><InfoRow label="Nama Sekolah" value={child.schoolName} /></div>
          ...
          {(child.fatherName || child.fatherIc || child.fatherPhone || child.fatherJob) && (
            <Section title="👨 Maklumat Bapa">
              <div className="col-span-2"><InfoRow label="Nama" value={child.fatherName} /></div>
          ...
          {(child.motherName || child.motherIc || child.motherPhone || child.motherJob) && (
            <Section title="👩 Maklumat Ibu">
              <div className="col-span-2"><InfoRow label="Nama" value={child.motherName} /></div>
      ...
      {activeTab === "growth" && (
        <div className="space-y-4">
          {!dataLoaded ? <div className="text-center py-12 text-gray-400">Memuatkan...</div>
          : growth.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-2xl border">
              <div className="text-5xl mb-4">📏</div>
              <p className="text-gray-500 font-medium">Belum ada rekod perkembangan</p>
              <Link href="/growth" className="inline-flex items-center gap-2 gradient-primary text-white px-5 py-2.5 rounded-xl mt-4 text-sm font-medium hover:opacity-90 transition">Tambah Rekod</Link>
            </div>
          ) : (
            <>
              {latestGrowth && (
              ...
              <div className="bg-white rounded-2xl border overflow-hidden">
                <div className="p-4 border-b"><h3 className="font-semibold text-gray-700">Sejarah Rekod</h3></div>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50"><tr>{["Tarikh","Berat","Tinggi","BMI"].map((h) => <th key={h} className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{h}</th>)}</tr></thead>
        ...
        <div className="space-y-3">
          ...
          : healthRecords.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-2xl border">
              ...
              <Link href="/health" className="inline-flex items-center gap-2 gradient-primary text-white px-5 py-2.5 rounded-xl mt-4 text-sm font-medium hover:opacity-90 transition">Tambah Rekod</Link>
            </div>
          ) : healthRecords.map((r) => {
              ...
              <div key={r.id} className="bg-white rounded-2xl border p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {r.nextAppointment && <div className="mt-1.5 inline-flex items-center gap-1 text-xs bg-blue-50 text-blue-700 px-2.5 py-1 rounded-full">🗓️ Temujanji: {formatDate(r.nextAppointment)}</div>}
                  </div>
                </div>
              </div>
            );
            ...
            <>
              ...
              {attendance.length === 0 ? (
                <div className="text-center py-12 bg-white rounded-2xl border">
                  ...
                  <Link href="/attendance" className="inline-flex items-center gap-2 gradient-primary text-white px-5 py-2.5 rounded-xl mt-4 text-sm font-medium hover:opacity-90 transition">Tambah Rekod</Link>
                </div>
              ) : (
                <div className="bg-white rounded-2xl border overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead className="bg-gray-50"><tr>{["Tarikh","Status","Nota"].map((h) => <th key={h} className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{h}</th>)}</tr></thead>
                      <tbody className="divide-y divide-gray-100">
        ...
        <div>
          ...
          : achievements.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-2xl border">
              ...
              <Link href="/achievements" className="inline-flex items-center gap-2 gradient-primary text-white px-5 py-2.5 rounded-xl mt-4 text-sm font-medium hover:opacity-90 transition">Tambah Pencapaian</Link>
            </div>
          ) : (
